...
export default function FichasPage() {
  ...
  async function fetchPacientes(query: string, p: number = 1) {
    ...
    try {
      ...
      if (!res.ok || json?.ok === false) throw new Error(json?.error || "Error al obtener fichas");
      setItems(json?.data || []);
      setPage(json?.page || 1);
  ...
  useEffect(() => { fetchPacientes("", 1); }, [especieFilter]);
  ...
  return (
    <div className="space-y-6 px-4 py-8">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowCreate(true)} className="px-5 py-3 rounded-lg font-semibold text-white bg-indigo-600 hover:bg-indigo-700">+ Crear Ficha</button>
      </div>
      ...
      <div className="bg-white rounded-xl shadow p-4">
        <div className="flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
              ...
              <div className="flex-shrink-0 relative">
                ...
                {speciesOpen && (
                  <div className="absolute right-0 mt-1 w-44 bg-white rounded-xl shadow border z-10 overflow-hidden" role="listbox">
                    {[{ key: '', label: 'Todos' }, { key: 'gato', label: 'Gatos' }, { key: 'perro', label: 'Perros' }].map((opt) => {
          ...
          <div className="text-sm text-gray-600">{loading && items.length === 0 ? "Cargando fichas..." : `${total} resultado(s)`}</div>
        </div>
      </div>

      {showCreate && (<FichaCreateModal isOpen={showCreate} onClose={() => setShowCreate(false)} onSaved={() => fetchPacientes("", 1)} />)}

      <div className="grid gap-4 sm:gap-6 grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
        {showSkeletons ? (
          Array.from({ length: 9 }).map((_, index) => (<PacienteSkeletonCard key={`skeleton-${index}`} />))
        ) : (
          items.map((p) => {
            ...
            return (
              <a href={`/admin/fichas/${p.mascotas_id}`} key={p.mascotas_id} className="group relative overflow-hidden rounded-2xl ring-1 ring-gray-200/70 bg-white/80 backdrop-blur-sm shadow-sm hover:shadow-xl hover:-translate-y-0.5 transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500">
      ...
      <div className="mt-8 flex items-center justify-center gap-2">
        <button className="px-3 py-2 rounded border bg-white disabled:opacity-50" onClick={() => fetchPacientes(search, Math.max(1, page - 1))} disabled={page <= 1 || loading}>Anterior</button>
        <div className="flex items-center gap-1">
          {Array.from({ length: totalPages }).slice(0, 7).map((_, i) => {
            const idx = i + 1;
            if (totalPages > 7 && idx > 5 && idx < totalPages) return null;
            return (<button key={idx} onClick={() => fetchPacientes(search, idx)} className={`min-w-9 h-9 px-3 py-2 rounded border text-sm ${page === idx ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white hover:bg-gray-50'}`} disabled={loading}>{idx}</button>);
          })}
          {totalPages > 7 && (<><span className="px-1">…</span><button onClick={() => fetchPacientes(search, totalPages)} className={`min-w-9 h-9 px-3 py-2 rounded border text-sm ${page === totalPages ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white hover:bg-gray-50'}`} disabled={loading}>{totalPages}</button></>)}
        </div>
        <button className="px-3 py-2 rounded border bg-white disabled:opacity-50" onClick={() => fetchPacientes(search, Math.min(totalPages, page + 1))} disabled={page >= totalPages || loading}>Siguiente</button>
      </div>

      {!loading && items.length === 0 && (<div className="text-center text-gray-500 mt-12">Sin fichas que coincidan con la búsqueda.</div>)}
    </div>
  );
}

// (migrado desde pacientes) – no reexportar nada
